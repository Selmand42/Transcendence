...
const createAccessToken = (payload) => jwt.sign({ ...payload, tokenType: 'access' }, env.jwtSecret, {
    expiresIn: ACCESS_TOKEN_TTL_SECONDS
});
const createRefreshToken = (payload) => jwt.sign({ ...payload, tokenType: 'refresh' }, env.jwtSecret, {
    expiresIn: REFRESH_TOKEN_TTL_SECONDS
});
const serializeCookie = (name, value, maxAge) => cookie.serialize(name, value, {
    httpOnly: true,
    maxAge,
    path: '/',
    sameSite: 'lax',
...
const appendCookies = (reply, cookiesToAdd) => {
...
const issueTokens = (reply, payload) => {
...
const clearOauthStateCookie = (reply) => {
    appendCookies(reply, [serializeCookie(GOOGLE_STATE_COOKIE, '', 0)]);
};
const decodeToken = (token, expectedType) => {
    ...
    const sub = typeof subRaw === 'number'
        ? subRaw
...
const normalizeNickname = (value) => value
    .toLowerCase()
    .replace(/[^a-z0-9]+/gi, '-')
    .replace(/^-+/, '')
    .replace(/-+$/, '')
    .slice(0, 32);
const ensureUniqueNickname = async (db, desired) => {
...
const buildFrontendRedirect = (status) => status ? `${FRONTEND_AUTH_PATH}?oauth=${status}` : FRONTEND_AUTH_PATH;
...
const createUniqueAlias = async (db, tournamentId, desiredAlias) => {
...
function registerAuthenticationHelpers(app) {
...
export const buildServer = () => {
    ...
    app.addHook('onRequest', (request, _reply, done) => {
        request[METRICS_START] = process.hrtime.bigint();
        done();
    });
    app.addHook('onResponse', (request, reply, done) => {
        const start = request[METRICS_START];
        if (start) {
    ...
    app.get('/api/users/oauth/google/callback', async (request, reply) => {
        const redirectWithStatus = (status) => reply.redirect(303, buildFrontendRedirect(status));
        ...
        let user = await request.server.db.get(`SELECT id, email, nickname, provider FROM users WHERE provider = 'google' AND provider_id = ?`, googleId);
        if (!user) {
            const existingEmailOwner = await request.server.db.get(`SELECT id, provider, nickname, provider_id FROM users WHERE email = ?`, email);
            if (existingEmailOwner && existingEmailOwner.provider === 'google') {
                ...
                if (needsUpdate) {
                  ...
                  WHERE id = ?
                `, googleId, OAUTH_SENTINEL, existingEmailOwner.id);
                    }
                    catch (syncError) {
              ...
              `, googleId, OAUTH_SENTINEL, existingEmailOwner.id);
                ...
                const nicknameSource = profile.given_name ?? profile.name ?? email.split('@')[0] ?? 'google-user';
                ...
                VALUES (?, ?, ?, 'google', ?)
              `, email, OAUTH_SENTINEL, nickname, googleId);
                    user = {
                        id: result.lastID ?? 0,
                        email,
                        nickname,
        ...
        if (!user) {
            request.log.error({ email, googleId }, 'Google OAuth akışında kullanıcı oluşturma/güncelleme tamamlanamadı');
            return redirectWithStatus('internal_error');
        }
        const payload = {
            sub: user.id,
    ...
    app.post('/api/users/register', { schema: manualRegisterSchema, preHandler: [registerRateLimiter] }, async (request, reply) => {
    ...
    app.post('/api/users/register/google', { schema: googleRegisterSchema, preHandler: [registerRateLimiter] }, async (request, reply) => {
    ...
    app.post('/api/users/login', { schema: manualLoginSchema, preHandler: [loginRateLimiter] }, async (request, reply) => {
        const { email, password } = request.body;
        const user = await request.server.db.get(`SELECT * FROM users WHERE email = ? AND provider = 'local'`, email);
        if (!user) {
            return reply.status(401).send({
    ...
    app.post('/api/users/login/google', { schema: googleLoginSchema, preHandler: [loginRateLimiter] }, async (request, reply) => {
        const { googleId } = request.body;
        const user = await request.server.db.get(`SELECT id, email, nickname, provider FROM users WHERE provider = 'google' AND provider_id = ?`, googleId);
        if (!user) {
    ...
    app.post('/api/users/refresh', { schema: refreshSchema, preHandler: [refreshRateLimiter] }, async (request, reply) => {
        const unauthorized = () => {
            clearSessionCookies(reply);
            return reply.status(401).send({
                error: 'Unauthorized',
        ...
        const user = await request.server.db.get(`SELECT id, email, nickname, provider FROM users WHERE id = ?`, session.sub);
        if (!user) {
            return unauthorized();
        }
        const payload = {
    ...
    app.post('/api/users/logout', { preHandler: [app.authenticate] }, async (_, reply) => {
        clearSessionCookies(reply);
        reply.status(204).send();
    });
    app.get('/api/users/me', { preHandler: [app.authenticate], schema: whoAmISchema }, async (request) => {
    ...
    app.get('/api/users/profile', { preHandler: [app.authenticate] }, async (request, reply) => {
        const session = request.session;
        if (!session) {
            return reply.status(401).send({
                error: 'Unauthorized',
    ...
    app.patch('/api/users/profile', { preHandler: [app.authenticate] }, async (request, reply) => {
        const session = request.session;
        if (!session) {
            return reply.status(401).send({
                error: 'Unauthorized',
        ...
        const duplicate = await request.server.db.get(`SELECT 1 FROM users WHERE LOWER(nickname) = LOWER(?) AND id != ?`, nickname, session.sub);
        if (duplicate) {
            return reply.status(409).send({
                error: 'NicknameTaken',
        ...
        await request.server.db.run(`UPDATE users SET nickname = ? WHERE id = ?`, nickname, session.sub);
    ...
    const fetchTournamentDTO = async (db, tournamentId) => {
    ...
    app.post('/api/tournaments', { preHandler: [app.authenticate] }, async (request, reply) => {
        const session = request.session;
        if (!session) {
            return reply.status(401).send({
                error: 'Unauthorized',
    ...
    app.get('/api/tournaments', { preHandler: [app.authenticate] }, async (request) => {
        const rows = (await request.server.db.all(`
        SELECT
          t.id,
          t.name,
          t.owner_id,
    ...
    app.post('/api/tournaments/:id/join', { preHandler: [app.authenticate] }, async (request, reply) => {
        const session = request.session;
        if (!session) {
            return reply.status(401).send({
                error: 'Unauthorized',
        ...
        try {
            ...
            VALUES (?, ?, ?, 0)
          `, tournamentId, session.sub, alias);
    ...
    app.post('/api/tournaments/:id/start', { preHandler: [app.authenticate] }, async (request, reply) => {
        const session = request.session;
        if (!session) {
            return reply.status(401).send({
                error: 'Unauthorized',
        ...
        const players = (await request.server.db.all(`SELECT alias, is_ai FROM tournament_players WHERE tournament_id = ? ORDER BY created_at ASC`, tournamentId));
        if (players.length < 2) {
            return reply.status(400).send({
