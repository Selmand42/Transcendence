...
const makeTournamentRoomCode = (tournamentId: number, matchId: string) => {
  // Turnuva maçı için özel oda kodu: tournament-{tournamentId}-{matchId}
  // matchId zaten benzersiz olduğu için bu kod da benzersiz olacak
...
const resetBall = (state: GameState) => {
  ...
  state.ball.dx = (totalScore % 4) < 2 ? -1 : 1; // 0-1: A'ya (-1), 2-3: B'ye (1)
  state.ball.dy = 0;
...
const handleGoal = async (room: Room, scoringIndex: 0 | 1) => {
  // Oyun zaten bitmişse işlem yapma
  if (room.gameEnded) {
    return;
  }
...
const tickRoom = (room: Room) => {
  ...
  // Oyun bitmişse tick'i durdur (ama state'i gönder)
  if (room.gameEnded) {
  ...
  // Oyun bitmişse goal kontrolü yapma
  if (!room.gameEnded) {
...
const bindClient = (socket: WebSocket, room: Room, playerIndex: 0 | 1) => {
  ...
  const handleClose = () => {
    ...
    // Eğer oda boşaldıysa veya oyun devam ediyorsa
    if (room.clients.length === 0) {
      // Oda boşaldı, odayı kapat
      teardownRoom(room.code);
    } else {
      // Bir oyuncu ayrıldı, diğer oyuncuya bildir
...
export const registerGameWebSocket = (app: FastifyInstance) => {
  ...
  wss.on('connection', (socket, request) => {
    ...
    const assignToRoom = (room: Room) => {
      ...
      // Oyun başlangıç zamanını ayarla (ilk oyuncu geldiğinde)
      if (room.clients.length === 0) {
        room.startTime = new Date();
      }
      // db'yi her zaman ayarla (eğer yoksa)
      if (!room.db) {
    ...
    socket.on('message', (data) => {
      ...
      // Input ve ping mesajları bindClient içinde handle ediliyor
      if (parsed.type === 'input' || parsed.type === 'ping') {
        // Bu mesajlar bindClient içindeki listener tarafından handle edilecek
        return;
      }

      // Eğer oyuncu zaten bir odada ise, create/join mesajlarını kabul etme
      if (activeRoom) {
      ...
      if (parsed.type === 'create_room') {
        ...
        // Turnuva maçı için özel oda kodu oluştur
    ...
    socket.on('close', () => {
      // close handler bindClient içinde handle ediliyor
      activeRoom = null;
      roomCode = null;
    });
