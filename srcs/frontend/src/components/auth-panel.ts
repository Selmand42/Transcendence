    ...
    if (response.ok) {
      ...
      if (contentType && contentType.includes('application/json')) {
        try {
          ...
          // JSON parse hatası - backend HTML döndürmüş olabilir, sessizce devam et
          clearSession();
        }
      } else {
...
export const createAuthPanel = () => {
  ...
  const setActiveSection = (targetId: string) => {
    ...
    sections.forEach((section) => {
      ...
      if (isActive) {
        // Inline style ile kesinlikle görünür yap
        section.style.display = 'block';
        section.classList.remove('hidden');
      } else {
        // Inline style ile kesinlikle gizle
        section.style.display = 'none';
  ...
  actionButtons.forEach((action) => {
    ...
    if (action.type === 'google') {
      ...
      // Normal tab butonları için başlangıç stili (pasif)
