...
class OnlineGameClient {
  ...
  private setStatus(message: string, type: 'info' | 'success' | 'error' = 'info') {
    ...
    if (type === 'success') {
      ...
      // Turnuva maçı varsa sarı renk, yoksa normal renk
  ...
  private startAnimation() {
    ...
    const render = () => {
      // Oyun bitti ise render loop'unu durdur
      if (this.gameEnded) {
        return;
      }
  ...
  private handleMessage(event: MessageEvent<string>) {
    ...
    if (parsed.type === 'room_joined') {
      ...
      // Turnuva maçı ise turnuva ismi ve round bilgisini göster
      if (this.tournamentMatchData) {
      ...
      // Turnuva maçı ise turnuvaya dön ekranı göster
      if (this.tournamentMatchData) {
      ...
      if (backButton && this.tournamentMatchData) {
        const tournamentId = backButton.dataset.tournamentId || this.tournamentMatchData.tournamentId;
        backButton.addEventListener('click', () => {
          // Maç sonucu gönderildikten sonra backend'in bracket'i güncellemesi için kısa bir gecikme
          setTimeout(() => {
            location.hash = `/tournament?tournament=${tournamentId}`;
          }, 500);
        });
  ...
  private connect(action: 'create' | 'join' | 'tournament', roomCode?: string) {
    ...
    this.ws.addEventListener('open', () => {
      if (action === 'create') {
        ...
        // Turnuva maçı için otomatik bağlan
        this.ws?.send(
          JSON.stringify({
...
export const initializeOnlineGame = (
  ...
  // Turnuva maçı varsa otomatik bağlan, yoksa normal mesaj göster
  if (tournamentMatchData) {
    // Turnuva maçı için butonları gizle ve otomatik bağlan
    controls.createButton.style.display = 'none';
    controls.joinButton.style.display = 'none';
    controls.roomInput.style.display = 'none';
    // Turnuva ismi ve round bilgisini göster
    ...
    // Kısa bir gecikme ile otomatik bağlan (WebSocket'in hazır olması için)
    setTimeout(() => {
